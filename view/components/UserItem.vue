<template>
  <div class="m-2 w-auto h-auto">
    ...
    <UDropdown
      ...
      :ui="{
        background: 'dark:bg-custom-950',
        width: 'w-auto',
        arrow: { background: 'dark:before:bg-custom-400' },
      }"
    ...
    >
      ...
      <div v-else class="w-auto flex justify-start items-center gap-2 p-4">
        <div
          class="rounded-full bg-custom-400 dark:bg-custom-500 h-10 w-10 flex justify-center items-center"
        >
          <img
            draggable="false"
            v-if="user.avatar"
            class="w-full h-full border-2 border-custom-400 rounded-full"
            :src="user.avatar"
          />
        ...
        <div class="border-l border-custom-400 pl-2 w-auto">
          <p
            class="tracking-wide lg:truncate lg:max-w-[150px] max-w-full capitalize"
          >
            {{ user.name }}
          </p>
          <p
            class="text-custom-500 dark:text-custom-400 font-semibold text-sm tracking-wider"
          >
            {{ user.role }}
          </p>
        </div>
    ...
    <!-- mobile view -->
    <div
      class="w-full border rounded-sm border-custom-600 flex justify-between lg:hidden"
    >
      <div
        class="w-auto flex justify-start items-center gap-2 p-4 cursor-pointer"
        @click="viewProfile"
      >
        <div
          v-if="
